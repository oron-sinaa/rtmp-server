      ...
      switch (e.type) {
        ...
        case "sublist": {
          ...
          sublist.unshift({
            ...
            placeholder: "none",
            help: "A convenient name to describe this "+e.itemLabel+". It won't be used by VISION.",
            pointer: {
              main: e.saveas,
              index: "x-LSP-name"
            }
              ...
              case 'streamname': {
                f = function(val,me) {
                  ...
                  if (val.replace(/[^\dA-z_\-\.]/g,'') != val) {
                    return {
                      msg: 'Special characters (except for underscores and dots) are not allowed.',
                      classes: ['red']
                    };
                  }
              ...
              case 'streamname_with_wildcard': {
                f = function(val,me) {
                  ...
                  if (streampart.replace(/[^\dA-z_\-\.]/g,'') != streampart) {
                    return {
                      msg: 'Special characters (except for underscores and dots) are not allowed in a stream name.',
                      classes: ['red']
                    };
                  }
  ...
  format: {
    ...
    status: function(item) {
      ...
      switch (item.online) {
        ...
        case  1: $s.text('Available').addClass('green'); break;
  ...
  showTab: function(tab,other) {
    ...
    if (mist.user.loggedin) {
      ...
      if (mist.data.config.serverid) {
        document.title = mist.data.config.serverid+" - VISION MI";
      }
    }
    ...
    switch (tab) {
        ...
        $main.append(UI.buildUI([
          {
            ...
            label: 'Host',
            help: 'Url location of the VISION API. Generally located at http://VISIONIP:4242/api',
      ...
      case 'Create a new account':
        UI.elements.menu.addClass('hide');
        $main.append(
          $('<p>').text('No account has been created yet in the VISION at ').append(
      ...
      case 'Overview':
        ...
        $main.append(UI.buildUI([
          {
            type: 'help',
            help: 'You can find most basic information about your VISION here.<br>You can also set the debug level and force a save to the config.json file that VISION uses to save your settings. '
          },{
            type: 'span',
            label: 'Version',
            pointer: {
            ...
            type: 'span',
            label: 'Licensed to',
            value: (mist.data.config.organization)
          },{
            type: 'span',
            label: 'License valid till',
            value: (mist.data.config.expirydate)
          },{
            type: 'span',
            ...
            type: 'span',
            label: 'Maximum allowed streams',
            value: (mist.data.config.maximumcameras)
          },{
            type: 'span',
          ...
          $("<br>"),
          $("<h3>").text("Write config to disk now"),
          {
            type: "help",
            help: "Tick the box in order to force an immediate save to the config.json VISION uses to save your settings. Saving will otherwise happen upon closing VISION. Don\'t forget to press save after ticking the box."
          },{
            type: 'checkbox',
        ...
        function updateStreams() {
          // var request = {active_streams: true};
            // totals:{
            //   fields: ['clients'],
            //   start: -2
            // },
        ...
        function enterStats() {
          ...
          $lastboottime.text(UI.format.dateTime(mist.data.config.lastboottime,'long'));
          ...
          for (var i in mist.data.log) {
            ...
            if (['FAIL','ERROR'].indexOf(l[1]) > -1) {
              ...
              $errors.append(
                $('<div>').append(
                  $('<span>').append(UI.format.time(l[0])).append(' - ')
                ).append(
                  $content
                ).css('width', '60em')
              );
        ...
        UI.interval.set(updateStreams,15e3);
      ...
      case 'General': {
        ...
        $main.html(UI.buildUI([
          $("<h2>").text("General settings"),{
            type: "help",
            help: "These are settings that apply to your VISION instance in general."
          },{
            type: 'str',
            label: 'Human readable name',
            pointer: {
            ...
            help: 'You can name your VISION here for personal use. You\'ll still need to set host name within your network yourself.'
          },{
            type: 'debug',
            label: 'Debug level',
            pointer: {
            ...
            help: 'You can set the amount of debug information VISION saves in the log. A full reboot of VISION is required before some components of VISION can post debug information.'
          },{
            type: "selectinput",
            label: "Access log",
            selectinput: [
              ["","Do not track"],
              ["LOG","Log to VISION log"],
              [{
                type:"str",
                label:"Path"
              },"Log to file"]
...

          {
            ...
            help: "Change the way the session token gets passed to and from VISION, which can be set as a cookie or URL parameter named `tkn`. Reading the session token as a URL parameter takes precedence over reading from the cookie.<br>Default: all"
          },
      ...
      case 'Streams':
        ...
        $main.append(
          $('<h3>').text('Connected Streams '+'('+(mist.data.active_streams ? mist.data.active_streams.length:0)+'/'+(mist.data.streams ? Object.keys(mist.data.streams).length : 0)+')')
        )
        $main.append(
          UI.buildUI([{
            type: 'help',
        ...
        function createPage(type,streams,folders) {
          ...
          switch (type) {
            ...
            case 'list':
            default: {
              var $tbody = $('<tbody>').append($('<tr>').append('<td>').attr('colspan',10).text('Loading..'));
        ...
        $main.append(UI.buildUI([
          {
            ...
            help: ("<p>\
                    Below is the explanation of the input methods for VISION. Anything between brackets () will go to default settings if not specified.\
                  </p>\
                  <table class=valigntop>\
                    ...
                    <tr>\
                      ...
                      <td>\
                        ...
                        Supported inputs are: DTSC, FLV, MP3. VISION Pro has TS, MP4, ISMV added as input.\
                      </td>\
                    </tr>\
                      <th>\
                        Folder\
                    ...
                    <tr>\
                      ...
                      <td>\
                        IP is white listed IP for pushing towards VISION, if left empty all are white listed.<br>\
                        PASSWORD is the application under which to push to VISION, if it doesn\'t match the stream will be rejected. PASSWORD is VISION Pro only.\
                      </td>\
                    </tr>\
                    <tr>\
                      <th>RTSP</th>\
                      <td>push://(IP)(@PASSWORD)</td>\
                      <td>IP is white listed IP for pushing towards VISION, if left empty all are white listed.</td>\
                    </tr>\
                    <tr>\
                      <th>TS</th>\
                    ...
                    <tr>\
                      ...
                      <td>MISTSERVER_IP is the IP of another VISION to pull from.<br>\
                        PORT is the DTSC port of the other VISION. (default is 4200)<br>\
                        STREAMNAME is the name of the target stream on the other VISION. If left empty, the name of this stream will be used.\
                      </td>\
                    </tr>\
                    <tr>\
                      <th>HLS</th>\
                      <td>http://URL/TO/STREAM.m3u8</td>\
                      <td>The URL where the HLS stream is available to VISION.</td>\
                    </tr>\
                    <tr>\
                      <th>RTSP</th>\
      ...
      case 'Embed':
        ...
        $embedlinks.append(
          ...
          $('<h3>').text('Urls'),
          {
            ...
            label: 'Controls',
            type: 'select',
            select: [['1','VISION Controls'],['stock','Player controls'],['0','None']],
            pointer: {
              main: custom,
              index: 'controls'
            },
      ...
      case 'Record':
        ...
        mist.send(function(d){
          ...
          $c.append(
            $('<h3>').text('Active File Recordings ' + '(' + ($push.find('tr').length-1) + '/' + (mist.data.active_streams ? mist.data.active_streams.length:0) + ')')
          )
          // .append(
          ...
          $c.append($push);
          $c.append(
            $('<h3>').text('Configured File Recordings')
          ).append(
          ...
          UI.interval.set(function(){
            ...
            },{push_list:1,push_auto_list:1,active_streams:1});
          },15e3);

        },{push_settings:1,push_list:1,push_auto_list:1,active_streams:1});
        ...
        function buildTheThings(edit) {
          ...
          var build = [{
            ...
            help: 'Where the stream will be pushed to.<br>\
                  ...
                  <ul>\
                    <li>$stream - inserts the stream name used to push to VISION</li>\
      ...
      case 'Triggers':
        ...
        $main.append(
          UI.buildUI([{
            type: 'help',
            help: 'Triggers are a way to react to events that occur inside VISION. These allow you to block specific users, redirect streams, keep tabs on what is being pushed where, etcetera. For full documentation, please refer to the developer documentation section on the VISION website.'
          }])
        ).append(
      ...
      case 'Logs':
        ...
        $main.append(UI.buildUI([{
          type: 'help',
          help: 'Here you have an overview of all edited settings within VISION and possible warnings or errors VISION has encountered. VISION stores up to 100 logs at a time.'
        },{
          label: 'Refresh every',
          type: 'select',
          select: [
          ...
          for (var index in logs) {
            ...
            $tbody.append(
              $('<tr>').html(
                ...
                $('<td>').html(color(logs[index][3])).css('text-align','center')
              ).append(
      ...
      case 'Statistics':
        ...
        mist.send(function(){
          ...
          $UI.html(UI.buildUI([{
            type: 'help',
            help: 'Here you will find the VISION stream statistics, you can select various categories yourself. All statistics are live: up to five minutes are saved.'
      ...
      case 'Server Stats':
        ...
        $main.append(
          UI.buildUI([{
            type: 'help',
            help: 'You can find general server statistics here. Note that memory and CPU usage is for your entire machine, not just VISION.'
          }])
        ).append(
      ...
      case 'Email for Help':
        ...
        $main.append(
          UI.buildUI([{
            type: 'help',
            help: 'You can use this form to email VISION support if you\'re having difficulties.<br>A copy of your server config file will automatically be included.'
          },{
            type: 'str',
            label: 'Your name',
            validate: ['required'],
    ...
    var obj = {
      ...
      error: function(jqXHR,textStatus,errorThrown){
        ...
        if (!opts.hide) {
          ...
          $('#message').addClass('red').text('An error occurred while attempting to communicate with VISION:').append(
            $('<br>')
          ).append(
      ...
      success: function(d){
        ...
        switch (d.authorize.status) {
          case 'OK':
            ...
            var save = $.extend({},d);
            var keep = ['config','capabilities','ui_settings','LTS','active_streams','browse','log','bandwidth']; //streams was already copied above
            for (var i in save) {
              if (keep.indexOf(i) == -1) {
                delete save[i];
              }
