...
namespace Controller{
  ...
  void setAgentMode(){
    const char* agentKey = getenv("AGENT");
    isAgentMode = (agentKey && std::string(agentKey) == "1");
  }
  ...
  void handleMsg(void *err){
    ...
    pthread_setname_np(pthread_self(), "TrLogParser");
#endif
  ...
  void writeConfigToDisk(){
    /// Do not write if mode is agent
    if (isAgentMode){
      FAIL_MSG("Writing to config is not allowed in Agent Mode");
      return;
    }
    JSON::Value tmp;
    ...
    if (Controller::hasEnoughFreeSpace("/", 1)){
      ...
      FAIL_MSG("[RTMPServer] Not enough space to safely write config to disk!");
    }
  }
  ...
  bool hasEnoughFreeSpace(const std::string path, double requiredPercentage){
    ...
    if (statvfs(path.c_str(), &stat) != 0){
      FAIL_MSG("Failed to get filesystem statistics for path: %s");
      return false;
    }

    // Calculate the percentage of free space
    double totalBlocks = static_cast<double>(stat.f_blocks);
    double freeBlocks = static_cast<double>(stat.f_bfree);
    double freePercentage = (freeBlocks / totalBlocks) * 100;

    // Check if the free space is greater than or equal to the required percentage
    return freePercentage >= requiredPercentage;
  }
