...
namespace Controller{
  ...
  tthread::recursive_mutex actPushMut; // mutex to sync activePushes map
    ...
    std::queue<pid_t> toWipe;
    {
      ...
      for (std::map<pid_t, JSON::Value>::iterator it = activePushes.begin(); it != activePushes.end(); ++it){
        if (Util::Procs::isActive(it->first)){
          ...
          toWipe.push(it->first);
        }
      }
    }
    while (!toWipe.empty()){
      removeActivePush(toWipe.front());
      toWipe.pop();
    }
    return false;
  }
  ...
  /// Loops, checking every fifth second if any pushes need restarting.
  void pushCheckLoop(void *np){
#ifdef WITH_THREADNAMES
    pthread_setname_np(pthread_self(), "TrPushChecker");
#endif
    readPushList();
    pushListRead = true;
    IPC::sharedPage pushPage("MstPush", 8 * 1024 * 1024, true, false);
    while (Controller::conf.is_active){
      std::set<std::string> activeStreams = Controller::getActiveStreams();
      if (activeStreams.size()){
        JSON::Value allAutopushes = Controller::Storage["autopushes"];
        jsonForEach(allAutopushes, it){
          std::string stream = (*it)[0u];
          std::string target = (*it)[1u];
          if ((it->size() >= 2) && (activeStreams.count(stream))){
            bool checkPushActive = isPushActive(stream, target);
            if (!checkPushActive){
      ...
      std::queue<pid_t> toWipe;
      {
        tthread::lock_guard<tthread::recursive_mutex> actGuard(actPushMut);
        for (std::map<pid_t, JSON::Value>::iterator it = activePushes.begin(); it != activePushes.end(); ++it){
          if (!Util::Procs::isActive(it->first)){toWipe.push(it->first);}
        }
      }
      while (!toWipe.empty()){
        removeActivePush(toWipe.front());
      ...
      // wait at least 10 seconds and also keep listening for controller interrupts
  ...
  // Just gives a list of all currently active pushes without restarting any needed pushes
  // Controller push does it anyway; this function with speed up the push_auto_list API
