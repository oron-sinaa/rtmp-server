...
std::string getChallenge(Socket::Connection &conn){
  ...
  // Refresh authentication token per hour
  Date << TimeInfo->tm_hour << "-" << TimeInfo->tm_mday << "-" << TimeInfo->tm_mon << "-" << TimeInfo->tm_year + 6479;
...
int Controller::handleAPIConnection(Socket::Connection &conn){
  ...
  pthread_setname_np(pthread_self(), "TrTcpApiHandler");
#endif
  // set up defaults
...
#ifdef WITH_THREADNAMES
  pthread_setname_np(pthread_self(), "TrUdpApiHandler");
#endif
...
void Controller::handleAPICommands(JSON::Value &Request, JSON::Value &Response){
  ...
  if (Request.isMember("license_limit")){
    int totalStreams = Controller::Storage["streams"].size();
    JSON::Value licenseLimit;
    licenseLimit["current_license_num"] = totalStreams;
    licenseLimit["maximum_license_num"] = Controller::maximumCameras;
    Response["license_limit"] = licenseLimit;
  }
  ...
  if (Request.isMember("addstream")){
    JSON::Value &in = Request["addstream"], &out = Controller::Storage["streams"];
    uint64_t newStreams = 0;
    jsonForEach(in, jit){
      if (!out.isMember(jit.key())){
        ++newStreams;
      }
    }
    if (newStreams > Controller::maximumCameras){
      FAIL_MSG("[RTMPServer] Rejecting addstream request for more than allowed streams %u > %u", in.size(), Controller::maximumCameras);
      Response["ActiveLicenseLimit"] = false;
    }else{
      Controller::AddStreams(Request["addstream"], Controller::Storage["streams"]);
      Response["ActiveLicenseLimit"] = true;
    }
  }
  if (Request.isMember("remaininglicenselimit")){
    int totalStreams = Controller::Storage["streams"].size();
    Response["remaininglicenselimit"] = Controller::maximumCameras - totalStreams;
  }
  ...
  if (!Request.isMember("minimal") || Request.isMember("config")){
    ...
    /// [RTMPServer] passing the @param expiryDate, organization, maximumCameras, expiryDate in response
    int diff = 0;
    if (Controller::timezone == "UTC"){diff = 19800;}
    Response["config"]["organization"] = (Controller::organization.size() == 0) ? "None" : Controller::organization;
    if (Controller::mode == "OFFLINE"){
      Response["config"]["expirydate"] = (Controller::expiryDate.size() == 0) ? "Unlimited" : Controller::epochToString(Controller::expiryDateInEpoch + diff);
    }
    else if (Controller::mode =="ONLINE"){
      Response["config"]["expirydate"] = (Controller::centralExpiry.size() == 0) ? "None" : Controller::centralExpiry;
    }
    Response["config"]["maximumcameras"] = (Controller::maximumCameras == -1) ? "Unlimited" : std::to_string(Controller::maximumCameras);
    if (!Response["config"].isMember("serverid")){Response["config"]["serverid"] = "";}
    Response["config"]["lastboottime"] = Controller::lastBootTime;
  }
  ...
  if (Request.isMember("api_endpoint")){
    int apiPort = Controller::Storage["config"]["controller"]["port"];
    if (!apiPort){apiPort = 4242;}
    std::string apiUrl = "http://localhost:" + std::to_string(apiPort);
    HTTP::URL url(apiUrl);
  ...
  // Just give a list of all currently active pushes without trying to wipe them from active pushes
  if (Request.isMember("push_list")){Controller::listPush(Response["push_list"], true);}
  ...
  if (Request.isMember("streams_status")){
    JSON::Value streams_status;
    Controller::getStreamData(streams_status);
    Response["streams_status"] = streams_status;
  }
  ...
  if (Request.isMember("bandwidth_data")){
    HIGH_MSG("[RTMPServer] Reading bandwidth data from SQL DB");
    JSON::Value json_resp;
    json_resp.null();
    if (Controller::conf.trafficConsumption){
      sqlite3 *db = nullptr;
      bool isLoaded = Database::loadDatabase(db);
      if (!isLoaded){ERROR_MSG("[RTMPServer] Could not open SQL DB on bandwidth_data API call");}
      if (isLoaded && db){
        JSON::Value streams = Controller::Storage["streams"];
        jsonForEach(streams, jit){
          JSON::Value columnData = Database::getColumn2(db, jit.key());
          EXTREME_MSG("[RTMPServer] columnData = %s", columnData.asString());
          json_resp[jit.key()] = columnData;
        }
        Database::closeDatabase(db);
      }
    }
    Response["traffic"] = json_resp;
  }
}
