...
/* User list for bandwidth statistics */
std::set<std::string> usersList;
bool streamPage = true;
bool UserPage = true;
int DBRegulator = 3600;
...
void Controller::SharedMemStats(void *config){
  ...
  pthread_setname_np(pthread_self(), "TrStatsChecker");
#endif
...
void Controller::statSession::finish(){
  ...
  if (Controller::accesslog.size()){
    // Log only if the connector name was found and ignore file recordings (HTTPTS)
    if (Controller::accesslog == "LOG" && (!curConnector.empty()) && (curConnector != "OUTPUT:HTTPTS")){
      std::stringstream accessStr;
      std::string shortSessId = sessId.substr(0, 5) + "..." + sessId.substr(sessId.length() - 5);
      accessStr << "Session <" << shortSessId << "> " << streamName << " (" << curConnector
...
std::string Controller::refactorStream(std::string streamName){
    int idx = streamName.find(".stream");
    if (idx != std::string::npos){
      std::string modifiedName = streamName.substr(0, idx);
      if (modifiedName.size() > 30){
       streamName = modifiedName.substr(modifiedName.size() - 30 ,modifiedName.size());
      }
    }
    return streamName;
}
...
void Controller::addStreamInSHM(bool appendMode, std::vector<std::string> &olderStreams, std::string pageName){
  Controller::trafficProtocol trafficProtocol;
  JSON::Value streams = Controller::Storage["streams"];
  if (!streams.size()){return;}
  std::map<std::string, std::pair<std::string, std::string>> shmTrafficData;
  sqlite3 *db = nullptr;
  Database::loadDatabase(db);
  IPC::sharedPage trafficSharedPage;
  IPC::semaphore writeLock;
  writeLock.open(SEMAPHORE_LOCK_WRITE_MODE, O_CREAT | O_RDWR, ACCESSPERMS, 1);
  if (!writeLock){
    FAIL_MSG("[addStreamInSHM], Could not open writelock to add stream in SHM page!");
  }
  trafficSharedPage.init(pageName, TRAFFIC_STATISTICS_INITSIZE, false, false);
  if (!trafficSharedPage || !trafficSharedPage.mapped){
    trafficSharedPage.init(pageName, TRAFFIC_STATISTICS_INITSIZE, true, false);
  }
  if (!trafficSharedPage.mapped){
    FAIL_MSG("Could not open memory page for traffic stats");
    Database::closeDatabase(db);
    return;
  }
  if (trafficSharedPage.mapped){
    Util::RelAccX tempTrafficAccX(trafficSharedPage.mapped, false);
    if (writeLock && appendMode){
      // Before closing the page in AddStreams API store the shm data in programmed memroy
      for (int i = 0; i < olderStreams.size(); ++i){
        std::string stream = olderStreams[i];
        std::string hls = "0.0";
        std::string ws = "0.0";
        Util::RelAccX traficGroupWrite = Util::RelAccX(tempTrafficAccX.getPointer(olderStreams[i]), false);
        if (traficGroupWrite.getFieldAccX(trafficProtocol.hls) && traficGroupWrite.getPointer(trafficProtocol.hls)){
          hls = std::string(traficGroupWrite.getPointer(trafficProtocol.hls));
        }
        if (traficGroupWrite.getFieldAccX(trafficProtocol.ws) && traficGroupWrite.getPointer(trafficProtocol.ws)){
          ws = std::string(traficGroupWrite.getPointer(trafficProtocol.ws));
        }
        shmTrafficData[stream] = {hls, ws};
      }

      writeLock.wait();
      tempTrafficAccX.setReload();
      trafficSharedPage.master = true;
      trafficSharedPage.close();
      trafficSharedPage.init(pageName, TRAFFIC_STATISTICS_INITSIZE, true, false);
      writeLock.post();
    }
    Util::RelAccX trafficAccX(trafficSharedPage.mapped, false);
    if (!trafficAccX.isReady()){
      jsonForEach(streams, jit){
        trafficAccX.addField(jit.key().c_str(), RAX_NESTED, DEFAULT_SESSION_COUNT);
      }
      trafficAccX.setRCount(1);
      trafficAccX.setEndPos(1);
      trafficAccX.setReady();
    }
    jsonForEach(streams, jit){
      Util::RelAccX traficGroup = Util::RelAccX(trafficAccX.getPointer(jit.key()), false);
      traficGroup.addField(trafficProtocol.ws, RAX_STRING);
      traficGroup.addField(trafficProtocol.hls, RAX_STRING);
      traficGroup.addField(IS_UPDATED, RAX_INT);
      traficGroup.setRCount(1);
      traficGroup.setEndPos(1);
      traficGroup.setReady();

      /// Load database and dump to SHM page only on boot
      if (!appendMode && Database::streamExists(db, jit.key())){
        std::map<std::string, std::string> columnData = Database::getColumn(db, jit.key());
        std::string trafficOrgWise = columnData["traffic"];
        nlohmann::json trafficData = nlohmann::json::parse(trafficOrgWise);
        std::string organization;
        if (pageName == TRAFFIC_ORG_STAQU){organization = "staqu";}
        else if (pageName == TRAFFIC_ORG_CLIENT){organization = "client";}
        trafficProtocol.hlsBandwidth = (std::to_string(static_cast<double>(trafficData[organization]["hls"])) != "NULL") ? (std::to_string(static_cast<double>(trafficData[organization]["hls"]))) : "0.0";
        trafficProtocol.wsBandwidth = (std::to_string(static_cast<double>(trafficData[organization]["ws"])) != "NULL") ? (std::to_string(static_cast<double>(trafficData[organization]["ws"]))) : "0.0";
      }
      if (writeLock){
        writeLock.wait();
        /// use already store olderstream traffic data in map \b shmTrafficData on in appendMode when new stream added
        if (appendMode && shmTrafficData.find(jit.key()) != shmTrafficData.end()){
          trafficProtocol.wsBandwidth = (shmTrafficData[jit.key()].second);
          trafficProtocol.hlsBandwidth = (shmTrafficData[jit.key()].first);
        }
        traficGroup.setString(trafficProtocol.ws, trafficProtocol.wsBandwidth);
        traficGroup.setString(trafficProtocol.hls, trafficProtocol.hlsBandwidth);
        traficGroup.setInt(IS_UPDATED, 0);
        writeLock.post();
      }
    }
  }
  Database::closeDatabase(db);
  trafficSharedPage.master = false;
}

void Controller::trafficThread(void *np){
#ifdef WITH_THREADNAMES
  pthread_setname_np(pthread_self(), "trTraffic");
#endif
  std::vector<std::string> olderStreams;
  Controller::addStreamInSHM(false, olderStreams, TRAFFIC_ORG_STAQU);
  Controller::addStreamInSHM(false, olderStreams, TRAFFIC_ORG_CLIENT);
  sqlite3 *db = nullptr;
  Database::loadDatabase(db);
  Controller::trafficProtocol trafficProtocol;
  while (true){
    IPC::sharedPage trafficPageReadModeI;
    IPC::sharedPage trafficPageReadModeII;
    JSON::Value streams = Controller::Storage["streams"];
    IPC::semaphore writeLock;
    writeLock.open(SEMAPHORE_LOCK_WRITE_MODE, O_CREAT | O_RDWR, ACCESSPERMS, 1);
    if (!writeLock){
      FAIL_MSG("[Statistics], Could not open writelock to add traffic from SHM to DB!");
    }
    trafficPageReadModeI.init(TRAFFIC_ORG_STAQU, TRAFFIC_STATISTICS_INITSIZE, false, false);
    trafficPageReadModeII.init(TRAFFIC_ORG_CLIENT, TRAFFIC_STATISTICS_INITSIZE, false, false);

    /** [RTMPServer] Storing traffic data with \b staqu & \b client organization in programmable memory */
    if (trafficPageReadModeI.mapped && trafficPageReadModeII.mapped){
      Util::RelAccX trafficAccXReadI(trafficPageReadModeI.mapped, false);
      Util::RelAccX trafficAccXReadII(trafficPageReadModeII.mapped, false);
      jsonForEach(streams, jit){
        std::string streamName = jit.key();
        Util::RelAccX traficGroupReadI = Util::RelAccX(trafficAccXReadI.getPointer(jit.key()), false);
        Util::RelAccX traficGroupReadII = Util::RelAccX(trafficAccXReadII.getPointer(jit.key()), false);
        std::string wsTempI = "0.0", wsTempII = "0.0";
        std::string hlsTempI = "0.0", hlsTempII = "0.0";
        std::string ndvrTempI = "0.0", ndvrTempII = "0.0";

        if (traficGroupReadI.getFieldAccX(trafficProtocol.ws) && traficGroupReadI.getPointer(trafficProtocol.ws)){
          wsTempI = std::string(traficGroupReadI.getPointer(trafficProtocol.ws));
        }
        if (traficGroupReadI.getFieldAccX(trafficProtocol.hls) && traficGroupReadI.getPointer(trafficProtocol.hls)){
          hlsTempI = std::string(traficGroupReadI.getPointer(trafficProtocol.hls));
        }

        if (traficGroupReadII.getFieldAccX(trafficProtocol.ws) && traficGroupReadII.getPointer(trafficProtocol.ws)){
          wsTempII = std::string(traficGroupReadII.getPointer(trafficProtocol.ws));
        }
        if (traficGroupReadI.getFieldAccX(trafficProtocol.hls) && traficGroupReadI.getPointer(trafficProtocol.hls)){
          hlsTempII = std::string(traficGroupReadII.getPointer(trafficProtocol.hls));
        }

        /** Adding the ndvr data as well from getting the DB with respect to stream \b ndvrTempI for STAQU, \b ndvrTempII for CLIENT */
        std::map<std::string, std::string> columnData = Database::getColumn(db, jit.key());
        std::string trafficOrgWise = columnData["traffic"];
        nlohmann::json trafficData = nlohmann::json::parse(trafficOrgWise);
        ndvrTempI = std::to_string(static_cast<double>(trafficData["staqu"]["ndvr"]));
        ndvrTempII = std::to_string(static_cast<double>(trafficData["client"]["ndvr"]));

        nlohmann::json data;
        data["staqu"]["hls"] = std::stof(hlsTempI);
        data["staqu"]["ws"] = std::stof(wsTempI);
        data["staqu"]["ndvr"] = std::stof(ndvrTempI);
        data["client"]["hls"] = std::stof(hlsTempII);
        data["client"]["ws"] = std::stof(wsTempII);
        data["client"]["ndvr"] = std::stof(ndvrTempII);

        if (traficGroupReadI.getInt(IS_UPDATED) || traficGroupReadII.getInt(IS_UPDATED)){
          Database::injectInDB(db, jit.key(), data.dump());
          writeLock.wait();
          traficGroupReadI.setInt(IS_UPDATED, 0);
          traficGroupReadII.setInt(IS_UPDATED, 0);
          writeLock.post();
          INFO_MSG("Data write successfully in DATABASE");
        }
      }
    }
    std::this_thread::sleep_for(std::chrono::seconds(SHM_TO_DB_TIMEOUT));
  }
  Database::closeDatabase(db);
}

void Controller::addUserInStreamCol(std::string userName, std::string streamName, std::set<std::string> &usersList){
  streamName = Controller::refactorStream(streamName);
  IPC::sharedPage trafficSharedPage;
  IPC::semaphore writeLock;
  std::set<std::string> streams = Controller::getStreamPageFields();
  writeLock.open(SEMAPHORE_LOCK_WRITE_MODE, O_CREAT | O_RDWR, ACCESSPERMS, 1);
  if (!writeLock){
    FAIL_MSG("[Statistics], Could not open writelock to add user in SHM!");
  }
  trafficSharedPage.init(TRAFFIC_STATISTICS, TRAFFIC_STATISTICS_INITSIZE, false, false);
  if (!trafficSharedPage || !trafficSharedPage.mapped){
    trafficSharedPage.init(TRAFFIC_STATISTICS, TRAFFIC_STATISTICS_INITSIZE, true, false);
  }
  if (!trafficSharedPage.mapped){
    FAIL_MSG("Could not open memory page for traffic stats");
    return;
  ...
  if (trafficSharedPage.mapped){
    Util::RelAccX tempTrafficAccX(trafficSharedPage.mapped, false);
    /* Gather the SHM data in-memory RAM {Local vars} */
    std::map<std::string, std::vector<std::pair<std::string, std::vector<std::string>>>> temporaryData;
    for (auto &jit : streams){
      if (tempTrafficAccX.getFieldAccX(jit) && tempTrafficAccX.getPointer(jit)){
        Util::RelAccX trafficStreamRow = Util::RelAccX(tempTrafficAccX.getPointer(jit), false);
        for (auto& user : usersList){
          std::vector<std::string> consumedData;
          if (trafficStreamRow.getFieldAccX(user) && trafficStreamRow.getPointer(user.c_str())){
            JSON::Value userStreamRow = JSON::fromString(trafficStreamRow.getPointer(user.c_str()));
            std::string hls = userStreamRow["hls"];
            std::string ws = userStreamRow["ws"];
            std::string ndvr = userStreamRow["ndvr"];
            consumedData.push_back(hls);
            consumedData.push_back(ws);
            consumedData.push_back(ndvr);
            temporaryData[jit].push_back({user, consumedData});
          }
          else if (streamName == jit and user == userName){
            std::string hls = "0.0";
            std::string ws = "0.0";
            std::string ndvr = "0.0";
            consumedData.push_back(hls);
    ...
    if (!trafficAccX.isReady()){
      for (auto &jit : streams){
        trafficAccX.addField(jit, RAX_NESTED, DEFAULT_ROW_CAPACITY);
      }
      trafficAccX.setRCount(1);
      trafficAccX.setEndPos(1);
      trafficAccX.setReady();
    ...
    for (auto &jit : streams){
      std::vector<std::pair<std::string, std::vector<std::string>>> usersData = temporaryData[jit];
      Util::RelAccX streamAccX = Util::RelAccX(trafficAccX.getPointer(jit), false);
      for (auto &userRow:usersData){
        streamAccX.addField(userRow.first, RAX_STRING, 512);
      }
      streamAccX.setRCount(1);
      streamAccX.setEndPos(1);
      streamAccX.setReady();
      for (auto &userRow:usersData){
        JSON::Value userData;
        userData["hls"] = userRow.second[0];
        userData["ws"] = userRow.second[1];
        userData["ndvr"] = userRow.second[2];
        streamAccX.setString((userRow.first), userData.toString());
      }
    }
  trafficSharedPage.master = false;
  }
...
void Controller::addStreamInSHM2(bool regulate){
  Controller::trafficProtocol trafficProtocol;
  IPC::sharedPage trafficSharedPage;
  IPC::semaphore writeLock;
  sqlite3 *db = nullptr;
  JSON::Value streams = Controller::Storage["streams"];
  if (Controller::Storage["streams"].size() == 0){return;}
  Database::loadDatabase(db);
  writeLock.open(SEMAPHORE_LOCK_WRITE_MODE, O_CREAT | O_RDWR, ACCESSPERMS, 1);
  if (!writeLock){
    FAIL_MSG("[addStreamInSHM], Could not open writelock to add stream in SHM!");
  }
  trafficSharedPage.init(TRAFFIC_STATISTICS, TRAFFIC_STATISTICS_INITSIZE, false, false);
  if (!trafficSharedPage || !trafficSharedPage.mapped){
    trafficSharedPage.init(TRAFFIC_STATISTICS, TRAFFIC_STATISTICS_INITSIZE, true, false);
  ...
  if (trafficSharedPage.mapped){
    ...
    Util::RelAccX trafficAccX(trafficSharedPage.mapped, false);
    if (!trafficAccX.isReady()){
      /** Append all the streams as primary row, @brief add null user in all stream to maintain the SHM capacity */
      std::string nullUser = "nullUser";
      jsonForEach(streams, jit){
        std::string modifiedStream = Controller::refactorStream(jit.key());
        trafficAccX.addField(modifiedStream, RAX_NESTED, DEFAULT_ROW_CAPACITY);
      }
      trafficAccX.setRCount(1);
      trafficAccX.setEndPos(1);
      trafficAccX.setReady();
      jsonForEach(streams, jit){
        std::string streamName = jit.key();
        std::string modifiedStream = Controller::refactorStream(jit.key());
        Util::RelAccX trafficStreamRow = Util::RelAccX(trafficAccX.getPointer(modifiedStream), false);
        std::map<std::string, std::vector<std::string>> usersData = Database::getDatabaseDump(db, streamName);
        for (auto &userRow : usersData){
          trafficStreamRow.addField(userRow.first, RAX_STRING, 512);
        }
        trafficStreamRow.setRCount(1);
        trafficStreamRow.setEndPos(1);
        trafficStreamRow.setReady();
        for (auto &userRow : usersData){
          JSON::Value userData;
          userData["hls"] = (regulate) ? "0.0" : userRow.second[0];
          userData["ws"] = (regulate) ? "0.0" : userRow.second[1];
          userData["ndvr"] = (regulate) ? "0.0" : userRow.second[2];
          trafficStreamRow.setString((userRow.first), userData.toString());
        }
      }
    }
  }
...
void Controller::createUserStreamPage(){
  /* create shm for users list */
  IPC::sharedPage userSharedPage;
  JSON::Value streams = Controller::Storage["streams"];
  std::set<std::string> streamsSet;
  sqlite3 *db = nullptr;
  Database::loadDatabase(db);
  std::set<std::string> users = Database::getUsersFromDB(db);
  userSharedPage.init(USER_LIST, USER_INITSIZE, false, false);
  if (!userSharedPage || !userSharedPage.mapped){
    userSharedPage.init(USER_LIST, USER_INITSIZE, true, false);
  }
  if (!userSharedPage.mapped){
    FAIL_MSG("Could not open memory page for users");
    Database::closeDatabase(db);
    return;
  }
  if (userSharedPage.mapped){
    Util::RelAccX userAccX(userSharedPage.mapped, false);
    if (!userAccX.isReady()){
      userAccX.addField("nullUser", RAX_STRING);
      for (auto &it : users){userAccX.addField(it, RAX_STRING);}
      userAccX.setRCount(1);
      userAccX.setEndPos(1);
      userAccX.setReady();
    }
  }
  userSharedPage.master = false;

  /* create shm for streams */
  IPC::sharedPage streamSharedPage;
  if (Controller::Storage["streams"].size() == 0) {
    streamPage = false;
    return;
  }
  streamSharedPage.init(STREAM_LIST, STREAM_INITSIZE, false, false);
  if (!streamSharedPage || !streamSharedPage.mapped){
    streamSharedPage.init(STREAM_LIST, STREAM_INITSIZE, true, false);
  }
  if (!streamSharedPage.mapped){
    FAIL_MSG("Could not open memory page for users");
    Database::closeDatabase(db);
    return;
  }
  if (streamSharedPage.mapped){
    Util::RelAccX streamAccX(streamSharedPage.mapped, false);
    if (!streamAccX.isReady()){
      jsonForEach(streams, jit){
        std::string modifiedStream = Controller::refactorStream(jit.key());
        streamAccX.addField(modifiedStream, RAX_512STRING);
      }
      streamAccX.setRCount(1);
      streamAccX.setEndPos(1);
      streamAccX.setReady();
  ...
  Database::closeDatabase(db);
  streamSharedPage.master = false;
}

std::set<std::string> Controller::getUserPageFields(std::string pageName, std::string userName){
  IPC::sharedPage sharedPage;
  IPC::semaphore writeLock;
  std::set<std::string> prevList;
  writeLock.open(SEMAPHORE_LOCK_WRITE_MODE, O_CREAT | O_RDWR, ACCESSPERMS, 1);
  if (!writeLock){
    FAIL_MSG("[Statistics], Could not open writelock to add user in SHM!");
  }
  sharedPage.init(pageName, USER_INITSIZE, false, false);
  if (!sharedPage || !sharedPage.mapped){
    sharedPage.init(pageName, USER_INITSIZE, true, false);
  }
  if (!sharedPage.mapped){
    FAIL_MSG("Could not open memory page for traffic stats");
    return prevList;
  }
  if (sharedPage.mapped){
    Util::RelAccX A(sharedPage.mapped, false);
    prevList = A.getAllFields();
    if (A.getPointer(userName) || userName == ""){return prevList;}
    else{
      prevList.insert(userName);
      if (writeLock){
        writeLock.wait();
        A.setReload();
        sharedPage.master = true;
        sharedPage.close();
        sharedPage.init(pageName, USER_INITSIZE, true, false);
        writeLock.post();
      }
      Util::RelAccX userAccX(sharedPage.mapped, false);
      if (!userAccX.isReady()){
        for (auto &it : prevList){userAccX.addField(it, RAX_STRING);}
        userAccX.setRCount(1);
        userAccX.setEndPos(1);
        userAccX.setReady();
      }
    sharedPage.master = false;
    }
  }
  return prevList;
}

void Controller::updateStreamInSHM(){
  ...
  writeLock.open(SEMAPHORE_LOCK_WRITE_MODE, O_CREAT | O_RDWR, ACCESSPERMS, 1);
  if (!writeLock){
    FAIL_MSG("[Statistics], Could not open writelock to update stream in SHM!");
  }
  sharedPage.init(STREAM_LIST, USER_INITSIZE, false, false);
  if (!sharedPage || !sharedPage.mapped){
    sharedPage.init(STREAM_LIST, USER_INITSIZE, true, false);
  }
  if (sharedPage.mapped){
    Util::RelAccX A(sharedPage.mapped, false);
    if (writeLock && streamPage){
      writeLock.wait();
      A.setReload();
      sharedPage.master = true;
      sharedPage.close();
    ...
    Util::RelAccX streamAccX(sharedPage.mapped, false);
    if (!streamAccX.isReady()){
      jsonForEach(streams, jit){
        std::string modifiedStream = Controller::refactorStream(jit.key());
        streamAccX.addField(modifiedStream, RAX_STRING);
      }
      streamAccX.setRCount(1);
      streamAccX.setEndPos(1);
      streamAccX.setReady();
    ...
    sharedPage.master = false;
    if (!streamPage){streamPage = true;}
  }
}

std::set<std::string> Controller::getStreamPageFields(){
  IPC::sharedPage streamSharedPage;
  IPC::semaphore writeLock;
  std::set<std::string> streamsList;
  streamSharedPage.init(STREAM_LIST, USER_INITSIZE, false, false);
  if (streamSharedPage.mapped){
    Util::RelAccX A(streamSharedPage.mapped, false);
    streamsList = A.getAllFields();
  }
  return streamsList;
}

void Controller::trafficThread2(void *np){
  sqlite3 *db = nullptr;
  std::string prevDate = Util::getDateOnlyString();
  bool isDbOpen = false;
  do{
    isDbOpen = Database::loadDatabase(db);
    if (!isDbOpen){
      FAIL_MSG("[RTMPServer] Failed to load SQL DB for traffic thread, retrying in 5 seconds...");
      Controller::sleepInSteps(5);
    }
  }while(!isDbOpen);
  Controller::createUserStreamPage();
  Controller::addStreamInSHM2(false);
  int regCounter = 0;
  while (Controller::conf.is_active){
    ...
    /* Regulate the SHM page after day */
    std::string curTime = Util::getDateOnlyString2();
    std::string date = Util::getDateOnlyString();
    if (prevDate != date){
      WARN_MSG("Traffic page regulated successfully");
      Controller::addStreamInSHM2(true);
      prevDate = Util::getDateOnlyString();
    }
    ...
    if (trafficSharedPage.mapped){
      ...
      /** Getting the refactor string */
      std::map<std::string, std::string> refactorMap;
      jsonForEach(allStreams, it){
        std::string modifiedStream = Controller::refactorStream(it.key());
        refactorMap[modifiedStream] = it.key();
      }

      usersList = Controller::getUserPageFields(USER_LIST, "");
      for (const std::string& jit : streams){
        if (tempTrafficAccX.getFieldAccX(jit) && tempTrafficAccX.getPointer(jit)){
          Util::RelAccX trafficStreamRow = Util::RelAccX(tempTrafficAccX.getPointer(jit), false);
          std::map<std::string, std::vector<std::string>> dbDump;
          if (refactorMap[jit].size() != 0){
            dbDump = Database::getDatabaseDump(db, refactorMap[jit]);
          }
          for (const std::string& user : usersList){
            JSON::Value userStreamRow;
            if (trafficStreamRow.getFieldAccX(user) && trafficStreamRow.getPointer(user.c_str())){
              userStreamRow = JSON::fromString(trafficStreamRow.getPointer(user.c_str()));
            }
            std::string hls = userStreamRow.isMember("hls") ? userStreamRow["hls"] : "0.0";
            std::string ndvr = userStreamRow.isMember("ndvr") ? userStreamRow["ndvr"] : "0.0";
            std::string ws = userStreamRow.isMember("ws") ? userStreamRow["ws"] : "0.0";
            // std::string date = Util::getDateOnlyString();

            if (dbDump.find(user) != dbDump.end()){ndvr = dbDump[user][2];}

            if (hls != "0.0" || ws != "0.0" || ndvr != "0.0"){
              if (refactorMap[jit].size() != 0){
                  Database::injectInDB2(db, refactorMap[jit], user, date, std::stod(hls), std::stod(ndvr), std::stod(ws));
              }else{
                  FAIL_MSG("Empty feed name : %s, not inject in database yet", refactorMap[jit].c_str());
              }
            }
          }
        }
    ...
    /** Regulating the db @brief delete 7 days previous data on hourly basis,  @todo use \b DATABASE_REGULATION */
    if (regCounter > DBRegulator){
      std::string query = "DELETE FROM trafficConsumption WHERE date < date('now', '-7 days');";
      bool res = Database::runQuery(db, query);
      if (res == false){FAIL_MSG("Unable to regulate the database [7 days rotator]");}
      else{WARN_MSG("Database is successfully regulated");}
      regCounter = 0;
    }
    regCounter += SHM_TO_DB_TIMEOUT;
    Controller::sleepInSteps(5);
  }
  Database::closeDatabase(db);
}