...
namespace Mist{
  ...
  Output::Output(Socket::Connection &conn) : myConn(conn), tkn(""){
    dataWaitTimeout = 12000;
    pushing = false;
    recursingSync = false;
    firstTime = 0;
    ...
    newUA = true;
    lastPushUpdate = 0;
    prevKeyTime = 0xFFFFFFFFFFFFFFFFull;
  ...
  bool Output::isRecording(){
    static bool isRecording = (bool)(config->hasOption("target") && config->getString("target").size());
    return isRecording;
  }
  ...
  void Output::initialize(){
    ...
    if (!meta || streamName.size() < 1){
      onFail("Cannot initialize output without meta! Bad input data?");
      return;
    }
    sought = false;
    /*LTS-START*/
  ...
  // Abort file recording if no tracks or users remain, or the stream had exited
  bool Output::abortFileRecording(){
  ...
  int Output::run(){
    ...
    if (isFileTarget()){
      DEVEL_MSG("File recording was started with %zu valid tracks, %zu users, keepGoing=%d", M.getValidTracks().size(), userSelect.size(), keepGoing());
      if (!streamName.size()){
      ...
      initialize();
      if (abortFileRecording()){
        INFO_MSG("File recording was aborted with %zu valid tracks, %zu users, keepGoing=%d", M.getValidTracks().size(), userSelect.size(), keepGoing());
        return 3;
      }
    ...
    while (keepGoing() && (wantRequest || parseData)){
      ...
      if (parseData){
        ...
        if (!wantRequest && !parseData){
          // Exit if a non-live input was attempted to be recorded to fMP4 format
          continue;
        }
  ...
  void Output::dropTrack(size_t trackId, const std::string &reason, bool probablyBad){
    if (isFileTarget() && M.getType(trackId) == "video"){
      for (std::map<size_t, Comms::Users>::iterator it = userSelect.begin(); it != userSelect.end(); ++it){
        buffer.dropTrack(it->first);
      }
      config->is_active = false;
      Util::logExitReason(ER_PROCESS_SPECIFIC, "Tried to drop tracks while recording");
      // Sleep to ensure that the input was closed before exiting
      onFinish();
      Util::sleep(2000);
      exit(1);
    }
  ...
  bool Output::prepareNext(){
    // Only allow a few times to do this
    static uint8_t bufferRetries = 50;
    if (!buffer.size() && --bufferRetries){
      thisPacket.null();
      if (bufferRetries){
        HIGH_MSG("Buffer completely played out, sleeping");
        Util::sleep(100);
      }else{
        FAIL_MSG("Buffer completely played out, exiting");
        Util::logExitReason(ER_CLEAN_EOF, "Buffer completely played out, exiting");
        config->is_active = false;
        return false;
      }
      return true;
    }
    // Reset bufferRetries for the upcoming packets
    bufferRetries = 50;
    // check if we have a next seek point for every track that is selected
    if (buffer.size() != userSelect.size()){
      INFO_MSG("Buffer/select mismatch: %zu/%zu - exiting.", buffer.size(), userSelect.size());
      thisPacket.null();
      Util::logExitReason(ER_CLEAN_EOF, "Buffer/select mismatch");
      config->is_active = false;
      return false;
    }
    ...
    for (; trackTries < buffer.size(); ++trackTries){
      ...
      if (nxt.offset >= curPage[nxt.tid].len ||
        ...
        if (nxt.offset >= curPage[nxt.tid].len){
          ...
          thisPacket.null();
          Util::logExitReason(ER_CLEAN_EOF, "Reading past end of page");
          config->is_active = false;
          return false;
        }else{
          ...
          thisPacket.null();
          Util::logExitReason(ER_CLEAN_EOF, "Invalid packet: no data");
          config->is_active = false;
          return false;
        }
        return false;
      ...
      if (curPage[nxt.tid].len > nxt.offset+preLoad.getDataLen()+20 && memcmp(curPage[nxt.tid].mapped + nxt.offset + preLoad.getDataLen(), "\000\000\000\000", 4)){
        ...
        if (!nextTime){
          ...
          thisPacket.null();
          Util::logExitReason(ER_CLEAN_EOF, "Next packet has no time");
          config->is_active = false;
          return false;
        }
        if (nextTime < nxt.time){
          WARN_MSG("next packet timestamp < current packet timestamp  - (%u) < (%u)");
          thisPacket.null();
          Util::logExitReason(ER_CLEAN_EOF, "Next packet has timestamp less than the current timestamp");
          config->is_active = false;
          return false;
        }
        break; //Packet valid!
      ...
      if (nextKeyPage != INVALID_KEY_NUM && nextKeyPage != currentPage[nxt.tid]){
        ...
        if (nextTime < nxt.time){
          ...
          thisPacket.null();
          Util::logExitReason(ER_CLEAN_EOF, "next packet was before the current packet");
          config->is_active = false;
          return false;
        }
        break;//Valid packet!
      ...
      // in sync mode, after ~120 seconds, give up and drop the track.
      if ((++emptyCount >= dataWaitTimeout)){
        thisPacket.null();
        Util::logExitReason(ER_CLEAN_EOF, "EOP: data wait timeout");
        config->is_active = false;
        return false;
      }
      //every ~5 second, check if the stream is not offline
      if (emptyCount % 100 == 0 && M.getLive() && Util::getStreamStatus(streamName) == STRMSTAT_OFF){
        thisPacket.null();
        Util::logExitReason(ER_CLEAN_EOF, "stream went offline");
        config->is_active = false;
        return false;
      }
    ...
    if (!thisPacket){
      thisPacket.null();
      Util::logExitReason(ER_CLEAN_EOF, "packet load failure");
      config->is_active = false;
      return false;
    }
    ...
    if (!userSelect[nxt.tid]){
      thisPacket.null();
      Util::logExitReason(ER_CLEAN_EOF, "track is not alive!");
      config->is_active = false;
      return false;
    }

    //Update keynum only after every ~5th second in the timestamp
    //We do this because DTSC::Keys is pretty CPU-heavy
    // TODO: Experimentally tweak (5000) and check reliability
    if ((prevKeyTime == 0xFFFFFFFFFFFFFFFFull) || (nxt.time > prevKeyTime + 5000)){
      prevKeyTime = nxt.time;
      uint32_t thisKey = M.getKeyNumForTime(nxt.tid, nxt.time);
  ...
  void Output::stats(bool force){
    ...
    DONTEVEN_MSG("Writing stats: %s, %s, %s, %" PRIu64 ", %" PRIu64, getConnectedHost().c_str(), streamName.c_str(),
    ...
    statComm.setLastSecond(thisPacket ? thisTime/1000 : 0);
  ...
  bool Output::allowPush(const std::string &passwd){
    ...
    if (!strmSource.size()){
      FAIL_MSG("Push rejected - stream %s not configured or unavailable (%s)", streamName.c_str(), myConn.getHost().c_str());
      pushing = false;
      return false;
    }
    ...
    if (meta){
      std::set<size_t> validTracks = meta.getValidTracks();
      for (std::set<size_t>::iterator it = validTracks.begin(); it != validTracks.end(); it++){
        std::string trackType = meta.getType(*it);
        if(trackType == "video"){
          FAIL_MSG("[RTMPServer] Push rejected - Found an existing video track")
          pushing = false;
          return false;
        }else if(validTracks.size() > 1 && (trackType == "video" || trackType == "audio")){
          FAIL_MSG("[RTMPServer] Push rejected - Found an existing video/audio track")
          pushing = false;
          return false;
        }
      }
  ...
  void Output::nukeStream(const std::string &streamName){
