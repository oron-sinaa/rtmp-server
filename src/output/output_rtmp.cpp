...
namespace Mist{
  ...
  // If using RTMP push, set recording format to TS (m3u8) for AutoPush :
  void OutRTMP::setRecordExt(std::string &sName){
    if (sName.empty()){
      INFO_MSG("[RECORD] No stream name found to configure recording format.");
      return;
    }
    INFO_MSG("[RECORD] Changing recording format to Ts (m3u8)");
    JSON::Value recExt;
    // Removing target with older target fMP4 (m3u) if present:
    recExt["push_auto_remove"].append(sName);
    std::string targetUrlString = "/ndvr/" + sName + "/index.m3u";
    recExt["push_auto_remove"].append(targetUrlString);
    Socket::UDPConnection uSock;
    uSock.SetDestination(UDP_API_HOST, UDP_API_PORT);
    uSock.SendNow(recExt.toString());
    recExt.null();

    // Adding push with new target TS (m3u8) :
    recExt["push_auto_add"]["stream"] = sName;
    recExt["push_auto_add"]["target"] = "/ndvr/" + sName + "/index.m3u8";
    uSock.SetDestination(UDP_API_HOST, UDP_API_PORT);
    uSock.SendNow(recExt.toString());
  }
  ...
  void OutRTMP::sendNext(){
    ...
    if (type == "audio" && M.trackLoaded(thisIdx)){ // Added track loaded check. But for now, audio is disabled (not setuped for testing purposes)
    ...
    if ((amfData.getContentP(0)->StrValue() == "publish")){
      ...
      if (amfData.getContentP(3)){
        ...
        if (firstRun){
          ...
          if (streamCfg.isMember("disable_audio")){
            if (!(needsAudio = !streamCfg["disable_audio"].asBool())){
              INFO_MSG("Rejecting audio stream as requested");
            }
          }
          firstRun = false;
        }
        // setRecordExt(streamName);
