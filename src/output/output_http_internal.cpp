...
namespace Mist{
  ...
  void OutHTTP::HTMLResponse(const HTTP::Parser & req, bool headersOnly){
    ...
    std::string hlsUrl = "/hls/" + streamName + "/playlist.m3u8";
  ...
  JSON::Value OutHTTP::getStatusJSON(std::string &reqHost, const std::string &useragent){
    ...
    /** adding the bandwidth consumption data in json_res corresponding to stream (calculate only if \b TRAFFIC_CONSUMPTION = ON) */
    ...
    if (trafficConsumption) {
      ...
      JSON::Value columnData = Database::getColumn2(db, streamName);
      json_resp["traffic"] = columnData;
    }
