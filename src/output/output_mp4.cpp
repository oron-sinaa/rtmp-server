...
namespace Mist{
  ...
  bool OutMP4::mp4Header(Util::ResizeablePointer & headOut, uint64_t &size, int fragmented){
    ...
    for (std::map<size_t, Comms::Users>::const_iterator it = userSelect.begin(); it != userSelect.end(); it++){
      ...
      MP4::EDTS edtsBox;
      MP4::ELST elstBox;
      // [fmp4] use the `else` content unconditionally, i.e. use only elstBox.setVersion(0) because we do not keep such long data
      if (tDuration >= 0xFFFFFFFFull || M.getFirstms(it->first) - firstms >= 0xFFFFFFFFull){
      ...
      if (!M.getLive() && M.getFirstms(it->first) != firstms){
        ...
        elstBox.setCount(1);
        // [fmp4] second arg must always be 1
      ...
      // [fmp4] use only the marked lines, useLargeBoxes was removed above
      if (useLargeBoxes){
        MP4::CO64 CO64Box;
    ...
    // [fmp4] use only the `if (fragmented)` condition
    if (fragmented){
      MP4::MVEX mvexBox;
      size_t curBox = 0;
      MP4::MEHD mehdBox;
      ...
      while (!sortSet.empty()){
        ...
        if (useLargeBoxes){// Re-using the previously defined boolean for speedup
          ...
          // [fmp4] use only this, useLargeBoxes was removed above
  ...
  void OutMP4::sendFragmentHeaderTime(uint64_t startFragmentTime, uint64_t endFragmentTime){
    ...
    // [fmp4] myConn.sendNow(data, data.length()) ?
...
*/
    ...
    if (M.getLive()){
      ...
      // [fmp4] this will actually send the header data on the connection in the new implementation
      /*
        Here, we collect 10 fragments and send data (if realtime is true, which is NOT!)
        This is done by setting and resetting the value of `nextHeaderTime`
        After 10 MP4 fragments are collected
      */
      if (thisPacket.getTime() >= nextHeaderTime){
        if (M.getLive()){
          // [fmp4] implement check by time before sending the data on the connection
          // if (thisPacket.getTime() - nextHeaderTime < 6)
          if (fragSeqNum > 10){
            // [fmp4] ignore liveSeek() because it will never return true
            // because we do not run in realTime mode (defined by stream's "Simulated Live" field on frontend)
            if (liveSeek()){
              nextHeaderTime = 0xffffffffffffffffull;
              return;
            }
      ...
      // [fmp4] run only the else condition
      ...
      if (webSock) {
        ...
        // [fmp4] instead do myConn.sendNow()
        // [fmp4] can implement buffering here
