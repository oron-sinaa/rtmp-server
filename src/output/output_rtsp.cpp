...
namespace Mist{
  ...
  void OutRTSP::incomingRTP(const uint64_t track, const RTP::Packet &p){
    sdpState.handleIncomingRTP(track, p, &meta, aacTrackAdded);
  }
  ...
  void OutRTSP::sendNext(){
    ...
    if (trackType == "audio" && !(targetParams["audio"] == "1" || targetParams["audio"] == "true")){
      return;
    }
    if (trackType == "video"){
    ...
    if (prevVGetTime && prevAGetTime && trackDiffAV > 30000){
      FAIL_MSG("[RTMPServer] Video and audio received more than 30 seconds apart in output RTSP! exiting.");
    ...
    if (!thisPacket.getData() || dataLen == 0){
      FAIL_MSG("`thisPacket.getData()` is invalid.");
      return;
    }

    double multiplier = SDP::getMultiplier(&M, thisIdx);
    if (multiplier <= 0.0){
      // impossible scenario - exit
      FAIL_MSG("[RTMPServer] Invalid output RTP multiplier (%u) for track (%u) with codec (%s) in output RTSP", multiplier, thisIdx, M.getCodec(thisIdx).c_str());
      onFinish();
      return;
    }
    uint64_t msTime = thisPacket.getTime();
    if (0 == videoFirstRtpMs && trackType == "video"){
      setSyncMs("video");
    }
    if (0 == audioFirstRtpMs && trackType == "audio"){
      setSyncMs("audio");
    }
    uint64_t rtpTimestamp;
    if (trackType == "video"){
      if (msTime < videoMilliSyncMs){
        // impossible scenario - exit
        FAIL_MSG("Invalid output RTP msTime (%u) on track (%s) with offset (%u) in output RTSP", M.getCodec(thisIdx).c_str(), msTime, videoMilliSyncMs);
        onFinish();
        return;
      }
      rtpTimestamp = (uint64_t)((msTime - videoMilliSyncMs) * multiplier + videoFirstRtpMs);
    }
    if (trackType == "audio"){
      if (msTime < audioMilliSyncMs){
        // impossible scenario - exit
        FAIL_MSG("Invalid output RTP msTime (%u) on track (%s) with offset (%u) in output RTSP", M.getCodec(thisIdx).c_str(), msTime, audioMilliSyncMs);
        onFinish();
        return;
      }
      rtpTimestamp = (uint64_t)((msTime - audioMilliSyncMs) * multiplier + audioFirstRtpMs);
    }
    if (rtpTimestamp > UINT32_MAX){
      // handled in rtp input yet re-check here
      FAIL_MSG("[RTMPServer] RTSP/RTP msTime rollover (%u) for %s in output RTSP", rtpTimestamp, M.getCodec(thisIdx).c_str());
      onFinish();
      return;
    }
    ...
    sdpState.tracks[thisIdx].pack.setTimestamp(rtpTimestamp);
  ...
  std::string OutRTSP::generateSDP(std::string reqUrl){
    ...
    for (std::set<size_t>::iterator it = validTracks.begin(); it != validTracks.end(); ++it){
      if (M.getType(*it) == "audio" && !(targetParams["audio"] == "1" || targetParams["audio"] == "true")){
        continue;
      }
  ...
  bool OutRTSP::handleTCP(){
    if (!encoderChecked && !aacTrackAdded){
      encoderChecked = true;
      JSON::Value streamCfg = Util::getStreamConfig(streamName);
      bool needsAudioEncoder = false;
      if (streamCfg.isMember("audio_encoder")){
        needsAudioEncoder = streamCfg["audio_encoder"].asBool();
      }
      if (needsAudioEncoder){
        std::set<size_t> validTracks = M.getValidTracks();
        bool hasAudio = false;
        for (std::set<size_t>::iterator it = validTracks.begin(); it != validTracks.end(); ++it){
          if (M.getType(*it) == "audio"){
            hasAudio = true;
            if (M.getCodec(*it) == "AAC"){
              WARN_MSG("[RTMPServer] AAC track already exists - skipping encoder");
              needsAudioEncoder = false;
            }
          }
        }
        if (!hasAudio){
          needsAudioEncoder = false;
          WARN_MSG("[RTMPServer] No audio track found - skipping encoder");
        }
        if (needsAudioEncoder){
          size_t tid = meta.addTrack();
  ...
  void OutRTSP::handleUDP(){
    ...
    for (std::map<uint64_t, SDP::Track>::iterator it = sdpState.tracks.begin(); it != sdpState.tracks.end(); ++it){
      if (M.getType(thisIdx) == "audio" && !(targetParams["audio"] == "1" || targetParams["audio"] == "true")){
        return;
      }
  ...
  void OutRTSP::setSyncMs(std::string type){
    if (type == "video"){
      videoMilliSyncMs = M.getMilliSyncMs(thisIdx);
      if (M.getFirstRtpMs(thisIdx)){
        videoFirstRtpMs = M.getFirstRtpMs(thisIdx)-1;
      }
      INFO_MSG("Set video ms offset to %u and first time to %u", videoMilliSyncMs, videoFirstRtpMs);
    }
    if (type == "audio"){
      audioMilliSyncMs = M.getMilliSyncMs(thisIdx);
      if (M.getFirstRtpMs(thisIdx)){
        audioFirstRtpMs = M.getFirstRtpMs(thisIdx)-1;
      }
      INFO_MSG("Set audio ms offset to %u and first time to %u", audioMilliSyncMs, audioFirstRtpMs);
    }
  }

  /// Disconnects the user
  bool OutRTSP::onFinish(){
    INFO_MSG("Tearing down RTSP session %s", HTTP_R.GetHeader("Session").c_str());
    if (myConn){
      HTTP_S.SetHeader("Session", HTTP_R.GetHeader("Session"));
      HTTP_S.SetHeader("Connection", "close");
      HTTP_S.SendResponse("200", "OK", myConn);
      myConn.close();
    }
    config->is_active = false;
