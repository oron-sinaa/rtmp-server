...
namespace Mist{
  ...
  uint16_t Input::getLock(IPC::semaphore &playerLock, IPC::semaphore &pullLock){
    if (streamName.size()){
      if (needsLock()){
  ...
  int Input::boot(int argc, char *argv[]){
    ...
    if (streamCfg.isMember("source") && streamCfg["source"].asString().substr(0, 7) == "rtsp://" && streamCfg.isMember("always_on") && !streamCfg["always_on"].asBool()){
      FAIL_MSG("Denying attempt to open a non-always-on RTSP stream");
      return 0;
    }
    ...
    uint16_t isLocked = getLock(playerLock, pullLock);

    if (isLocked == 0){
      DEVEL_MSG("[RTMPServer] Acquired semaphore lock for input stream");
      config->activate();
    }else{
      Util::logExitReason(ER_SIGILL, "Failed to acquire semaphore lock for input stream");
      return exitAndLogReason();
    }
    ...
    while (config->is_active){
      uint16_t isLockRevoked = getLock(playerLock, pullLock);
      if (isLocked != 0){
        WARN_MSG("[RTMPServer] Exiting - semaphore lock for input stream was revoked");
        return isLockRevoked;
      }
      ...
      int status;
      uint64_t childStartTime = Util::bootSecs();
      ...
      uint64_t childEndTime = Util::bootSecs();
      // Re-fork and restart the input process after waiting for 10 seconds if set to always-on
      if (config->getString("input").substr(0, 7) == "rtsp://" && isAlwaysOn(true)){
        uint64_t childRunTime = childEndTime - childStartTime;
        if (childRunTime < 60){
          WARN_MSG("[RTMPServer] Waiting 30 seconds before restarting failing child pull process");
          Util::sleep(30000);
        }else{
          Util::sleep(1000);
        }
    ...
    if (pullLock){
      INFO_MSG("Unlinking pull semaphore lock on exit");
    ...
    if (playerLock){
      INFO_MSG("Unlinking player semaphore lock on exit");
  ...
  bool Input::isAlwaysOn(bool refresh){
    static bool wasAlwaysOn = false;
    static bool checkedOnce = false;
    if (!refresh){
      if (wasAlwaysOn){
        // Return early if already determined to be always on and no refresh is requested
        return true;
      }
      if (checkedOnce){
        // Return early if already determined to be not be always on and no refresh is requested
        return false;
      }
    }
    // Fetch the stream configuration
    JSON::Value streamCfg = Util::getStreamConfig(streamName);
    // Check the "always_on" member in the configuration
    if (streamCfg.isMember("always_on") && streamCfg["always_on"].asBool()){
      wasAlwaysOn = true;
      return true;
    }
    checkedOnce = true;
  ...
  // Checks in the server configuration if always_on was unset or if stream source has been changed/removed
  // Returns true if the above two conditions are true, false otherwise
  // Note: Avoid calling in loop!
  bool Input::shouldExit(){
    JSON::Value streamCfg = Util::getStreamConfig(streamName);
    static std::string prevSource = streamCfg["source"].asString();
    if (!streamCfg.isMember("source")){
      WARN_MSG("[RTMPServer] Removal of source in configuration requires exit");
      return true;
    }
    if (prevSource != streamCfg["source"].asString()){
      WARN_MSG("[RTMPServer] Change of source in configuration requires exit");
      return true;
    }
    if (isAlwaysOn() && !isAlwaysOn(true)){
      WARN_MSG("[RTMPServer] always_on set to false - exiting");
      return true;
    }
    return false;
  }
  ...
  void Input::serve(){
    ...
    while (keepRunning()){
      ...
      if (M.getLive() && !internalOnly){
        ...
        if (M.getValidTracks().size()){activityCounter = Util::bootSecs();}
      }
  ...
  bool Input::keepRunning(){
    ...
    static bool lastOnCheck = Util::bootSecs();
    ...
    /*LTS-END*/
    if (Util::bootSecs() - lastOnCheck >= 10){
      lastOnCheck = Util::bootSecs();
      if (shouldExit()){
        config->is_active = false;
        return false;
      }
  ...
  void Input::stream(){
    ...
    if (isSingular()){
      ...
      if (!Util::startInput(streamName, "push://INTERNAL_ONLY:" + config->getString("input"), true,
        ...
        FAIL_MSG("Could not start buffer, cancelling");
        return;
      }
    }else{
    ...
    userSelect.clear();
    finish();
    if (Util::Procs::isRunning(bufferPid)){
      Util::Procs::Stop(bufferPid);
      bufferPid = 0;
    }
    return;
  }
  ...
  void Input::removeTrack(size_t tid){
  ...
  void Input::removeUnused(bool removeAll){
...
//        meta.setIvec(tNumber, 0x5DC800E53A65018A);//Dash_compat
  ...
  bool Input::keepAlive(){
    static uint64_t lastOnCheck = Util::bootSecs();
