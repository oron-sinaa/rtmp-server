...
namespace Mist{
  inputBuffer::inputBuffer(Util::Config *cfg) : Input(cfg){
    ...
    option["short"] = "d";
    option["help"] = "Disable audio from stream";
    config->addOption("disable_audio", option);
    capa["optional"]["disable_audio"]["name"] = "Disable Audio";
    capa["optional"]["disable_audio"]["help"] = "Disable audio from stream";
    ...
    bufferTime = DEFAULTBUFFERSIZE;
    cutTime = 0;
    segmentSize = 1900;
    hasPush = false;
    everHadPush = false;
    resumeMode = false;
    maxDataWait = std::max((bufferTime / 1000), (uint64_t)30);
  }
  ...
  void inputBuffer::onCrash(){
    DEVEL_MSG("Cleaning buffer leftovers");
  ...
  bool inputBuffer::removeKey(size_t tid){
    ...
    return meta.removeFirstKey(tid, streamName);
  }
  ...
  void inputBuffer::removeUnused(bool removeAll){
    meta.reloadReplacedPagesIfNeeded();
    if (!meta){
      return;
    }
    ...
    if (removeAll){
      for (std::set<size_t>::iterator idx = tracks.begin(); idx != tracks.end(); idx++){
        ...
        while (keys.getValidCount() > 1){
          meta.removeFirstKey(i, streamName);
        }
      }
      return;
    }
    ...
    for (std::set<size_t>::iterator idx = tracks.begin(); idx != tracks.end(); ++idx){
      size_t i = *idx;
      if ((time - M.getLastUpdated(i)) > 2){continue;}
    ...
    for (std::set<size_t>::iterator idx = tracks.begin(); idx != tracks.end(); ++idx){
      ...
      if (M.getType(i) == "video" && M.getFirstms(i) < videoFirstms){
        videoFirstms = M.getFirstms(i);
      }

      uint64_t lastms = M.getLastms(i);
      /*
        if not updated for 30 seconds or an entire buffer duration (whichever is larger),
        or last updated track and this track differ by an entire buffer duration,
        and if this is the video track then erase all track and close the input;
      */
      bool isDataWaitExceeded = (time - lastUp > maxDataWait);
      bool isActiveTypeRelevant = compareLast && activeTypes.count(type);
      bool isTimeGapLargeEnough = (time - lastUp > 5);
      bool isBeforeBuffer = (compareLast < firstms) && ((firstms - compareLast) > bufferTime);
      bool isAfterBuffer = (compareFirst > lastms) && ((compareFirst - lastms) > bufferTime);
      bool isBufferTimeExceeded = isBeforeBuffer || isAfterBuffer;

      if (isDataWaitExceeded || (isActiveTypeRelevant && isTimeGapLargeEnough && isBufferTimeExceeded)){
        // warn about erasing this track
        if (isDataWaitExceeded){
        ...
        meta.reloadReplacedPagesIfNeeded();
        Util::logExitReason(ER_CLEAN_EOF, "dead %s input stream with codec %s", type.c_str(), codec.c_str());
        // set status to Unavailable and shutdown the input
