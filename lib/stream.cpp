...
JSON::Value Util::getStreamConfig(const std::string &streamname){
  JSON::Value result;
  if (streamname.size() > 150){
    FAIL_MSG("Stream opening denied: %s is longer than 150 characters (%zu).", streamname.c_str(),
             streamname.size());
    return result;
  }
...
/// Does stream name sanitation first, followed by a stream name length check (<= 150 chars).
  ...
  */
  ...
  while (!streamAlive(streamname) && ++waiting < 240){
    ...
    if (!Util::Procs::isRunning(pid)){
      DEVEL_MSG("Input process (PID %d) shut down before stream coming online, aborting.", pid);
      break;
    }
  }
...
std::set<size_t> Util::wouldSelect(const DTSC::Meta &M, const std::map<std::string, std::string> &targetParams,
  ...
  if (targetParams.count("audio_track") && targetParams.at("audio_track").size()){
    if (targetParams.at("audio_track") != "-1" && targetParams.at("audio_track") != "none"){
      std::set<size_t> tracks = Util::findTracks(M, capa, "audio", targetParams.at("audio_track"), UA);
      result.insert(tracks.begin(), tracks.end());
    }
    noSelAudio = true;
  }
  if (targetParams.count("video_track") && targetParams.at("video_track").size()){
    if (targetParams.at("video_track") != "-1" && targetParams.at("video_track") != "none"){
      std::set<size_t> tracks = Util::findTracks(M, capa, "video", targetParams.at("video_track"), UA);
      result.insert(tracks.begin(), tracks.end());
    }
    noSelVideo = true;
  }
  if (targetParams.count("subtitle_track") && targetParams.at("subtitle_track").size()){
    std::set<size_t> tracks = Util::findTracks(M, capa, "subtitle", targetParams.at("subtitle_track"), UA);
    result.insert(tracks.begin(), tracks.end());
    noSelSub = true;
  }
  if (targetParams.count("meta_track") && targetParams.at("meta_track").size()){
    std::set<size_t> tracks = Util::findTracks(M, capa, "meta", targetParams.at("meta_track"), UA);
    result.insert(tracks.begin(), tracks.end());
    noSelMeta = true;
  }
  /*LTS-END*/
