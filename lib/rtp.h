...
#define PAYLOAD_TYPE_RTCP(x) ((x) >= (72) && (x) <= (76))
#define TIME_UNTIL_ROLLOVER(pTime, multiplier) \
  ((double)((uint64_t)UINT32_MAX - (pTime)) / ((double)(multiplier) * 1000.0))
...
namespace RTP{
  ...
  public:
    ...
    int64_t getRealTimeStamp() const;
    uint64_t generateTimeStamp() const;
  ...
  public:
    ...
    void addRTP(const RTP::Packet &rPkt, DTSC::Meta *meta, bool audioEncoder = false);
  ...
  public:
    ...
    void handleHEVC(uint64_t msTime, char *pl, uint32_t plSize, bool missed, DTSC::Meta *meta);
    void handleHEVCSingle(uint64_t ts, const char *buffer, const uint32_t len, bool isKey, DTSC::Meta *meta);
