...
namespace SDP{
  ...
  void State::parseSDP(const std::string &sdp){
    ...
    while (std::getline(ss, to, '\n')){
      ...
      if (to.substr(0, 2) == "m="){
        ...
        if (getline(words, item, ' ')){
          ...
          case 32: // MPV
            ...
            break;
          case 35: // H264 (BOSCH!)
            INFO_MSG("H264 payload type. BOSCH!");
            HIGH_MSG("Remapping m=video 0 RTP/AVP 35->97. BOSCH!");
            nope = false;
            avp_type = 97;
      ...
      if (to.substr(0, 8) == "a=rtpmap"){
        ...
        // convert to fullcaps
        if (to.substr(0, 12) == "a=rtpmap:102" && (trCodec == "H265")){
          HIGH_MSG("Remapping custom SDP param a=rtpmap:102 H265/90000 to H264. BOSCH!");
          trCodec = "H264";
        }
        for (unsigned int i = 0; i < trCodec.size(); ++i){
          if(trCodec[i] <= 122 && trCodec[i] >= 97){
            trCodec[i] -= 32;
          }
        }
        ...
        if (!myMeta->getCodec(tid).size()){
          ERROR_MSG("Unsupported RTP mapping: %s, removing track", mediaType.c_str());
          myMeta->removeTrack(tid);
          tracks.erase(tid);
        }else{
          tConv[tid].setProperties(*myMeta, tid);
      ...
      if (to.substr(0, 10) == "a=control:"){
        if (to[10] == '*'){
          // flawed - because no tid was assigned yet?
          WARN_MSG("[RTMPServer] Skipping a=control:* track from SDP");
          tracks.erase(tid);
          continue;
        }
        // first insert in the track
  ...
  void State::handleIncomingRTP(const uint64_t track, const RTP::Packet &pkt, DTSC::Meta *meta, bool audioEncoder){
    tConv[track].setCallbacks(incomingPacketCallback, snglStateInitCallback);
    tConv[track].addRTP(pkt, meta, audioEncoder);
  }
