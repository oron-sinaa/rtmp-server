...
void Socket::Connection::open(std::string address, bool nonblock){
  ...
  if (r == 0){
    ...
    lastErr = strerror(errno);
    WARN_MSG("Could not connect to %s! Error: %s", address.c_str(), lastErr.c_str());
    close();
  }
}
...
void Socket::Connection::open(std::string host, int port, bool nonblock, bool with_ssl){
  ...
  if (s != 0){
    ...
    WARN_MSG("Could not connect to %s:%i! Error: %s", host.c_str(), port, lastErr.c_str());
    close();
    return;
  }
  ...
  if (rp == 0){
    WARN_MSG("Could not connect to %s! Error: %s", host.c_str(), lastErr.c_str());
    close();
  }else{
    if (nonblock){
      int flags = fcntl(sSend, F_GETFL, 0);
...
void Socket::Connection::SendNow(const char *data, size_t len){
  // Temporarily set the socket to blocking mode if it's not already
  const bool wasBlocking = isBlocking();
  if (!wasBlocking){
    setBlocking(true);
  }
  size_t totalWritten = 0;
  while (totalWritten < len && connected()){
    size_t chunkSize = std::min(static_cast<size_t>(len - totalWritten), static_cast<size_t>(SOCKETSIZE));
    size_t written = iwrite(data + totalWritten, chunkSize);
    if (written == 0){
      // Avoid busy-wait if no progress is made
      if (!connected()){
        break;
      }
      Util::sleep(1);
...
void Socket::Connection::SendNow(const char *data, size_t len, uint16_t rateLimit){
...
unsigned int Socket::Connection::iwrite(const void *buffer, int len){
  ...
  if (skipCount > 0){
    unsigned int toSkip = std::min(static_cast<unsigned int>(len), skipCount);
    skipCount -= toSkip;
    if (toSkip == len){
      return len; // Entire chunk is skipped
    }
    buffer = static_cast<const char *>(buffer) + toSkip;
    len -= toSkip;
  }
  int result = isTrueSocket ? send(sSend, buffer, len, 0) : write(sSend, buffer, len);
  if (result <= 0){
    if (result == 0 || errno == EWOULDBLOCK || errno == EINTR) {
      return 0;
    }
    Error = true;
    lastErr = strerror(errno);
