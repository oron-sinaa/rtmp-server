...
namespace Util{
  ...
  size_t ResizeablePointer::getSize(){
    return currSize;
  }
  ...
  RelAccX::RelAccX(char *data, bool waitReady){
    ...
    if (isReady()){
      ...
      if (offset < 11 || offset >= getOffset()){
        VERYHIGH_MSG("Invalid field offset: %u", offset);
        p = 0;
        return;
      }
  ...
  std::set<std::string> RelAccX::getAllFields() const{
  ...
  void RelAccX::deleteRecords(uint32_t amount, std::string streamName){
    ...
    *hdrDeleted += amount; // update deleted record counter
    if (*hdrPresent >= amount){
      *hdrPresent -= amount; // decrease records present
      ...
      // Nuke stream for clean exit instead of unrecoverable exit(1)
      std::deque<std::string> command;
      command.push_back("/usr/local/bin/MistUtilNuke");
