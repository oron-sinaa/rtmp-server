...
namespace DTSC{
  ...
  void Meta::addTrackFrom(const DTSC::Scan &trak){
    ...
    if (trak.hasMember("firstrtpms")){
      setFirstRtpMs(tIdx, trak.getMember("firstrtpms"));
    }
    if (trak.hasMember("millisyncms")){
      setMilliSyncMs(tIdx, trak.getMember("millisyncms").asInt());
    }
  ...
  void Meta::streamInit(size_t trackCount){
    if (isMaster){
      ...
      trackList.addField("millisyncms", RAX_64UINT);
      trackList.addField("firstrtpms", RAX_32UINT);
    ...
    trackMilliSyncMsField = trackList.getFieldData("millisyncms");
    trackFirstRtpMsField = trackList.getFieldData("firstrtpms");
  }
  ...
  void Meta::refresh(){
    ...
    for (size_t i = 0; i < trackList.getPresent(); i++){
      ...
      t.partsTimeField = t.parts.getFieldData("time");
      t.partIsKeyframeField = t.parts.getFieldData("isKeyframe");
  ...
  bool Meta::reloadReplacedPagesIfNeeded(){
    ...
    if (stream.isReload() || stream.isExit()){
      ...
      if (!streamPage.mapped){
        INFO_MSG("Page %s not found", pageName);
        // Set to null pointers if we cannot reload those pages
        stream = Util::RelAccX();
  ...
  void Meta::merge(const DTSC::Meta &M, bool deleteTracks, bool copyData){
    ...
    for (std::set<size_t>::iterator it = editedTracks.begin(); it != editedTracks.end(); it++){
      ...
      setMilliSyncMs(newIdx, M.getMilliSyncMs(*it));
      setFirstRtpMs(newIdx, M.getFirstRtpMs(*it));
  ...
  void Meta::resizeTrack(size_t source, size_t fragCount, size_t keyCount, size_t partCount, size_t pageCount, const char * reason){
    ...
    Util::FieldAccX origPartTimeAccX = origParts.getFieldAccX("time");
    Util::FieldAccX origPartIsKeyframeAccX = origParts.getFieldAccX("isKeyframe");
    ...
    Util::FieldAccX partTimeAccX = t.parts.getFieldAccX("time");
    Util::FieldAccX partIsKeyframeAccX = t.parts.getFieldAccX("isKeyframe");
    ...
    for (size_t i = firstPart; i < endPart; i++){
      ...
      partTimeAccX.set(origPartTimeAccX.uint(i), i);
      partIsKeyframeAccX.set(origPartIsKeyframeAccX.uint(i), i);
    }
  ...
  size_t Meta::addTrack(size_t fragCount, size_t keyCount, size_t partCount, size_t pageCount, bool setValid){
    ...
    if (!isMemBuf){
      ...
      if (stream.isExit()){
        trackLock.post();
        WARN_MSG("Not adding track: stream is shutting down");
        return INVALID_TRACK_ID;
      }
    }
  ...
  void Meta::initializeTrack(Track &t, size_t fragCount, size_t keyCount, size_t partCount, size_t pageCount){
    ...
    t.parts.addField("time", RAX_64UINT);
    t.parts.addField("isKeyframe", RAX_UINT);
  ...
  void Meta::setMilliSyncMs(size_t trackIdx, const uint64_t &milliSyncMs){
    trackList.setInt(trackMilliSyncMsField, milliSyncMs, trackIdx);
  }
  uint64_t Meta::getMilliSyncMs(size_t trackIdx) const{
    return trackList.getInt(trackMilliSyncMsField, trackIdx);
  }

  void Meta::setFirstRtpMs(size_t trackIdx, const uint32_t &firstRtpMs){
    trackList.setInt(trackFirstRtpMsField, firstRtpMs, trackIdx);
  }
  uint64_t Meta::getFirstRtpMs(size_t trackIdx) const{
    return trackList.getInt(trackFirstRtpMsField, trackIdx);
  }
  ...
  bool Meta::removeFirstKey(size_t trackIdx, std::string streamName){
    ...
    t.parts.deleteRecords(deletedPartCount, streamName);
    ...
    if (firstKey + keyCount <= deletedKeyNum + 1){
      ...
      tPages.deleteRecords(1, streamName);
  ...
  void Meta::update(uint64_t packTime, int64_t packOffset, uint32_t packTrack, uint64_t packDataSize,
    ...
    t.parts.setInt(t.partsTimeField, packTime, newPartNum);
    t.parts.setInt(t.partIsKeyframeField, isKeyframe, newPartNum);
    if (newPartNum){
  ...
  uint64_t Meta::getSendLen(bool skipDynamic, std::set<size_t> selectedTracks) const{
    ...
    for (std::map<size_t, Track>::const_iterator it = tracks.begin(); it != tracks.end(); it++){
      ...
      if (!selectedTracks.size() || selectedTracks.count(it->first)){
        ...
        dataLen += 43; // firstRtpMs + milliSyncMs
      }
    }
    /*
    if (sourceURI.size()){
  ...
  void Meta::send(Socket::Connection &conn, bool skipDynamic, std::set<size_t> selectedTracks, bool reID) const{
    ...
    for (std::set<size_t>::const_iterator it = selectedTracks.begin(); it != selectedTracks.end(); it++){
      ...
      conn.SendNow("\000\012firstrtpms\001", 13);
      conn.SendNow(c64(getFirstRtpMs(*it)), 8);
      conn.SendNow("\000\013millisyncms\001", 14);
      conn.SendNow(c64(getMilliSyncMs(*it)), 8);
  ...
  /// Returns the index of the first part which is a keyframe.
  /// Return 0 if the given key part could not be found at the provided timestamp
  size_t Meta::getFirstKeyPart(uint64_t timestamp, size_t idx) const{
    if (idx == INVALID_TRACK_ID){return 0;}

    DTSC::Parts Parts(parts(idx));
    size_t firstPart = Parts.getFirstValid();
    size_t endPart = Parts.getEndValid();
    for (size_t i = firstPart; i < endPart; i++){
      if (Parts.isKeyframe(i) && Parts.getTimestamp(i) == timestamp){
        return i;
      }
    }
    INFO_MSG("[RTMPServer] Found no keyframe part for timestamp %u :(", timestamp);
    if (firstPart == 0){
      return 0;
    }
  ...
  /// Returns the index of the part just before a keyframe part.
  size_t Meta::getPrevToKeyPartTimeIndex(uint64_t timestamp, size_t idx) const{
    if (idx == INVALID_TRACK_ID){return 0;}

    DTSC::Parts Parts(parts(idx));
    size_t firstPart = static_cast<ssize_t>(Parts.getFirstValid());
    size_t endPart = static_cast<ssize_t>(Parts.getEndValid());
    // TODO: Implement binary search and/or data partitioning
    for (size_t i = endPart-1; i > firstPart; --i){
      if (Parts.isKeyframe(i) && Parts.getTimestamp(i) == timestamp){
        return i-1;
      }
    }
    INFO_MSG("[RTMPServer] Found no part previous to keyframe part with timestamp %u :(", timestamp);
    return INVALID_RECORD_INDEX;
  }
  ...
  Parts::Parts(const Util::RelAccX &_parts) : parts(_parts){
    ...
    timeField = parts.getFieldData("time");
    isKeyframeField = parts.getFieldData("isKeyframe");
  }
  ...
  uint64_t Parts::getTimestamp(size_t idx) const{return parts.getInt(timeField, idx);}
  bool Parts::isKeyframe(size_t idx) const{return parts.getInt(isKeyframeField, idx);}
