...
namespace SDP{
  ...
  bool Media::parseRtpMapLine(const std::string &line){
    ...
    if ((line.substr(0, 12) == "a=rtpmap:102") && (encodingName == "H265")){
  ...
  std::string Answer::toString(){
    ...
    for (size_t i = 0; i < sdpOffer.medias.size(); ++i){
      ...
      if ((videoLossPrevention & SDP_LOSS_PREVENTION_ULPFEC) && fmtRED && fmtULPFEC){
        if (fmtULPFEC->payloadType == 35){
          addLine("a=rtpmap:97 ulpfec/90000");
          addLine("a=rtpmap:97 red/90000");
        }else{
      ...
      if (fmtMedia->encodingName == "H264"){
        ...
        if (fmtMedia->payloadType == 35){
          HIGH_MSG("Remapping a=fmtp:35->97. BOSCH!")
          addLine("a=fmtp:97 profile-level-id=42e01f;level-asymmetry-allowed=1;packetization-mode=1");
        }else{
