...
namespace IPC{
  ...
  void sharedPage::close(){
    unmap();
    if (handle > 0){
      DONTEVEN_MSG("Closing page %s in %s mode", name.c_str(), master ? "master" : "client");
#if defined(__CYGWIN__) || defined(_WIN32)
      CloseHandle(handle);
#else
      ::close(handle);
  ...
  void sharedPage::init(const std::string &name_, uint64_t len_, bool master_, bool autoBackoff){
    ...
    mapped = 0;
    if (name.size()){
      DONTEVEN_MSG("Opening page %s in %s mode %s auto-backoff", name.c_str(),
      ...
      if (handle == -1){
        if (master){
          if (len > 1){INFO_MSG("Overwriting old page for %s", name.c_str());}
